...
def fetch_optimizer(lr, weight_decay, epsilon, max_iters, params):
    weight_decay = float(weight_decay)  # Ensure `weight_decay` is a float
    optimizer = torch.optim.AdamW(params, lr=lr, weight_decay=weight_decay, eps=epsilon)
...
def run_model(model, loss_fn, map_seg_loss_fn, d, device='cuda:0', sw=None, use_radar_encoder=None,
    ...
    vox_util = utils.vox.Vox_util(
        ...
        scene_centroid=scene_centroid.to(device),       # 三维坐标，表示原点
        bounds=bounds,                                  # tensor([[0., 1., 0.]])
        assert_cube=False)                              # (-50, 50, -5, 5, -50, 50)
    ...
    # 执行模型，得到分割结果
    seg_e = model(
        rgb_camXs=rgb_camXs,            #   相机RGB数据  -0.5, 0.5  像素范围        ([2, 6, 3, 224, 448])  批量大小为 2，6 个相机的 RGB 数据，每个图像有 3 个通道
        pix_T_cams=pix_T_cams,          #   相机内参的转换矩阵                      (2, 6, 4, 4)
        cam0_T_camXs=cam0_T_camXs,      #   相机的旋转和平移矩阵                    (2, 6, 4, 4)
        vox_util=vox_util,              #   体素工具，处理雷达数据
        rad_occ_mem0=in_occ_mem0)       #   雷达数据                               (2,1,200,8,200)
    ...
    if train_task == 'both' or train_task == 'object':
        if train_task == 'both':
            obj_seg_bev_e = seg_e[:, -1].unsqueeze(dim=1)  # ([2, 200, 200])  -->  ([2, 1, 200, 200])
        else:  # 'object'
            obj_seg_bev_e = seg_e
        ...
        # 模型预测 与 真实输出 在有效区域内的交集，对各个维度求和，得到每个batch的交集总和
...
def main(
    ...
    B = batch_size
    if len(device_ids) > 1:
        assert (B % len(device_ids) == 0), f"Batch size {B} must be divisible by the number of GPUs {len(device_ids)}"
    if grad_acc > 1:
    ...
    if model_type == 'transformer':
        model = SegnetTransformerLiftFuse(Z_cam=200, Y_cam=8, X_cam=200, Z_rad=Z, Y_rad=Y, X_rad=X, vox_util=None,
                                          ...
                                          freeze_convnext=freeze_convnext, learnable_fuse_query=learnable_fuse_query)
        ...
        model = SegnetWithMap(Z, Y, X, vox_util=vox_util, use_radar=use_radar,
                              ...
                              train_task=train_task, freeze_convnext=freeze_convnext)
...
if __name__ == '__main__':
    ...
    yaml_file_path = "/media/HDD1/users/user1/argho/BEVCar-main/configs/train/train_bevcar.yaml"
